...
- Variable importance shows.

```{r}
ggplot(imp)+geom_col(aes(x=variables,y=MeanDecreaseAccuracy,fill=variables))+coord_flip()+labs(title="Variable Importance",y=" Mean decrease in Accuracy", x= " Variable")+scale_y_continuous(labels = scales::percent_format())
```
...
ggplot(VI_plot)+geom_col(aes(x=(rownames(VI_plot)),y=imp.0,fill=rownames(VI_plot)))+coord_flip()+guides(fill=FALSE)+
